...
//var_dump($this->db);
    ...
    function update($data,$table,$updateItem,$updateId){
        ...
        foreach ($data as $key=>$value){
            $escStr = $this->db->quote(trim($value),PDO::PARAM_STR);;
        ...
        if ($res!==false){
			...
			$tmp = $this->db->errorInfo();
			$this->log->logData('Chyba SQL: ' . $query . ' Error: ' . $tmp[2],false,"error in update",true);
            return array("status"=>false,"result"=>$tmp[2]);
        }
    ...
    function row($query)
    {
        ...
        if ($results !== FALSE){
            ...
            $tmp = $this->db->errorInfo();
            $this->log->logData($query.", ".$tmp[2],false,"error in row");
            return false;
        }
    }
    ...
    function buildSql($string,$data=array())
    {
        ...
        foreach ($data as $key=>$value){
            ...
            if (preg_match($pattern1,$string)){
                $replacement = $this->db->quote($value,PDO::PARAM_STR);
